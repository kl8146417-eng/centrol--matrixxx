...
Launch specialist agents **in a single message** (all at once). Each agent is a `code-reviewer` type.

#### Agent Output Protocol

Every specialist agent receives this protocol alongside its domain instructions. Include it verbatim in each agent prompt:

```
PROJECT CONTEXT:
{project_profile as YAML}

FILES TO ANALYZE:
{scoped_files}

OUTPUT RULES:
- Only report issues with confidence >= 80%. No nitpicks, no style comments, no subjective preferences.
- Maximum 10 issues per agent — prioritize by impact. If you found more, keep only the 10 most impactful.
- Filter through PROJECT CONTEXT: never flag patterns that match documented project conventions.
- For EACH issue, use this exact structure:
  ...
  - **Code**:
    ```typescript
    {3-8 lines of the problematic code}
    ```
  - **Problem**: {1-2 sentences — what's wrong}
  - **Impact**: {why this matters — what breaks, degrades, or confuses}
  - **Fix**:
    ```typescript
    {concrete replacement code}
    ```
  - **Source**: {reference file or rule that justifies this finding}

If no issues found, say "No issues found from {domain} perspective."
```
...
> Analyze imports in `{scoped_files}`. For each import, determine source layer and target layer from the path (`core/`, `feature/`, `pattern/`, `ui/`, `layout/`, `styles/`). Flag any forbidden dependency per the architecture reference. Check for cross-feature imports (`feature/x` importing from `feature/y`).
> Load: `references/angular-architecture.md`
> Follow: **Agent Output Protocol**
...
### Consensus Lenses (deep mode only)

In `--deep` mode, the two highest false-positive domains — **Signals & Reactivity** and **AI Slop Detection** — use multi-lens consensus instead of a single agent. This is inspired by the Crab Review method: multiple agents analyze the same code through different analytical lenses, and only issues where 2+ agents agree get reported.

**Replace Agent 2** with 3 signal lenses:

| Lens | Agent | Focus |
|------|-------|-------|
| Signal Purist | 2a | Strict signal-first: every Observable that could be a signal is wrong |
| Pragmatic Migrator | 2b | Balanced: flag only clear wins, respect existing RxJS where it's reasonable |
| RxJS Defender | 2c | Conservative: only flag RxJS when it's objectively worse (BehaviorSubject as local state) |

Each lens agent gets the same files, same reference (`references/angular-anti-patterns.md` section 2), and the RxJS nuance instructions. They differ in their analytical stance — explicitly stated in their prompt.

**Replace Agent 8** with 3 slop lenses:

| Lens | Agent | Focus |
|------|-------|-------|
| Minimalist | 8a | Ruthless: any code that could be shorter without losing clarity is slop |
| Team Reader | 8b | Balanced: only flag code that would genuinely confuse a mid-level developer |
| Intent Checker | 8c | Conservative: only flag code where the "why" is clearly AI artifact, not intentional verbosity |

Each lens agent gets the same files and `references/ai-slop-detection.md`.

**Consensus algorithm** (applied per domain before team lead):

1. Group findings by `file:line_range` + issue concept (same location, same fundamental problem)
2. Count agreements:
   - **3/3 lenses** → CONFIRMED — include in team lead report
   - **2/3 lenses** → LIKELY — include in team lead report
   - **1/3 lenses** → NOISE — dismiss, do not forward to team lead
3. Annotate each finding with `[3/3]` or `[2/3]` for team lead visibility

This adds +4 agents in deep mode (from 10 to 14 total, before team lead). The cost is justified: these two domains produce the most false positives in practice.

In standard mode (no `--deep`), Agent 2 and Agent 8 run as single agents — no consensus overhead.
...
- The **Agent Output Protocol** (structured issue format with snippet, impact, and proposed fix)
...
- Instruction:
  ...
  > 3. **Cross-cluster boost** — if the same code location is flagged by 2+ different clusters, upgrade its severity by one level. Independent cluster convergence is strong signal.
  > 4. **Identify systemic patterns** — if every file has the same DI problem, that's a systemic issue, not 10 individual issues
  > 5. **Prioritize by business impact** — a bug that affects users is more critical than a naming convention
  > 6. **Cap at 30** — report top 30 issues. Note total if more.
  > 7. **Add a systemic diagnosis** — what is the ROOT CAUSE behind the pattern of issues? (e.g., "This codebase was written pre-signals and partially migrated. The root fix is a systematic signal migration, not patching individual files.")
  >
  > Output:
...
> 4. **Cross-domain boost** — if the same code location (`file:line_range`) is flagged by 2+ different domain specialists, **upgrade its severity by one level** (Minor → Important, Important → Critical). This is a strong signal — independent experts found the same problem from different angles.
> 5. **Cross-cutting observations** — look for issues no single specialist caught: inconsistent patterns across files, missing shared abstractions where 2+ files do the same transformation, naming inconsistencies within the scope
> 6. **Prioritize** — sort by impact: Critical (bugs, security, architecture) → Important (patterns, signals) → Minor (naming, style)
> 7. **Cap at 30** — report top 30 issues. If more exist, note the total.
>
> Output format — preserve the structured issue format from specialists:
>
> For each issue:
> ```
> ## #{N} — {Issue Title}
> - **Location**: `file:line`
> - **Severity**: Critical | Important | Minor {+ " ⬆ cross-domain boost" if upgraded}
> - **Category**: {domain}
> - **Code**: {snippet}
